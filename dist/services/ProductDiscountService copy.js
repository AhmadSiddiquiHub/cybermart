...
let ProductDiscountService = class ProductDiscountService {
    ...
    async findDiscountPricee(vendorProductVariantId, todaydate) {
        ...
        query.andWhere('(productDiscount.startDate <= :todaydate)', { todaydate });
        query.andWhere('(productDiscount.endDate is null OR productDiscount.endDate >= :todaydate)', { todaydate });
    ...
    async findAllVariantByIdd(id, todaydate) {
        ...
        query.select(selects);
        query.where('pd.vendorProductVariantId = :id ', { id });
        query.andWhere('pd.startDate <= :todaydate ', { todaydate });
        query.andWhere('(pd.endDate is null OR pd.endDate >= :todaydate)', { todaydate });
        query.orderBy('pd.price', 'ASC');
...
//# sourceMappingURL=ProductDiscountService%20copy.js.map